...
> Project Scope: `StarCluster 10,000-Star N-Body Simulation & Economy`
...
We actively maintain and provide critical security updates for the following release lines of **Jirnyak/starcluster**:
...
Security engineering in Jirnyak/starcluster is guided by the following domain-specific threat vector analyses:

### 1. Gravitational Singularity NaN Cascades
* **Description**: Zero-distance star collisions causing division by zero in acceleration vectors.
...
### 2. LocalStorage State Forgery
* **Description**: Tampering with credit balance or hyperdrive engine parameters via browser devtools.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
### 3. Main Thread CPU Starvation
* **Description**: Falling back to O(N^2) direct force computation instead of octree approximations.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
...
If you discover a security flaw or exploit vector in **Jirnyak/starcluster**, do **NOT** post it publicly in open issues or discussions.
