...
//
// ⚠️ (§55) «ВРАТЬ БОЛЬШЕ НЕЧЕМ» ОКАЗАЛОСЬ НЕПРАВДОЙ, и замер это показал.
// Модель полёта та же, но КОРАБЛЬ в ней — идеализированный: опорный «Hauler»
// идёт по маршруту, который ему можно пройти напрямую, а настоящий возчик того
// же класса скачет по узлам. Замер (`journey_probe`, сид 1234, 1200 звёзд, 60
// рейсов, 123 предложения): отношение «данный срок / МОЙ реальный полёт» в
// среднем **0.743**, и **84 предложения из 123 (68%) невыполнимы уже в момент,
// когда их предлагают**. Ровно та же болезнь, что §23 считал вылеченной, только
// причина другая — не формула, а корабль в ней.
//
// Проверены и отброшены замером две гипотезы: у опорного корпуса пустые баки
// (залил половину — 0.2%) и залил полностью (те же 0.2%). Масса ни при чём.
//
// Поэтому запас берётся не «сверх опорного рейса», а сверх РЕАЛЬНОГО: 1.35 × 1.82
// (замеренная оптимистичность опорного корабля) ≈ 2.5. Приёмка — доля
// невыполнимых в момент предложения, см. §55.
const double CONTRACT_DEADLINE_SLACK = 2.5;
...
// (§50) ПОСЛЕДНИЙ РУБЕЖ, а не срок ожидания.
//
// До §50 это был весь механизм: подождал шесть лет — приехал безличный буксир.
// Теперь ожидание физическое (свет дошёл + спасатель долетел), а это число
// осталось страховкой на случай, когда физика не справилась: борт унесло за
// край скопления, все живые слишком медленны, казна пуста. Срок задран до века
// именно затем, чтобы страховка была ВИДНА в замере: если она срабатывает
// часто — сломана физика, а не константа. Замер частоты — `strandStatLastResort`.
const double TOW_WAIT_YEARS = 100.0;
...
// (§50) Награда спасателю: доля цены корпуса спасённого, платит клиринговая
// палата (§47). Решение пользователя (§48.8, п.5) — «как есть, без сверки с
// Cr/год»: спасение не должно быть промыслом, оно должно окупать крюк.
const double RESCUE_HULL_SHARE = 0.10;
// НИЖНИЕ ГРАНИЦЫ допуска на встречу — не сам допуск. Настоящий критерий
// шагозависим («до цели меньше одного шага сближения И разницу скоростей можно
// снять одним импульсом»), см. `chaseShipToward`; эти числа лишь не дают ему
// схлопнуться в ноль на бесконечно малом шаге. Оба взяты у уже принятых в игре
// порогов: 0.01 ly — допуск швартовки у звезды в `moveShipToward`, 0.0001 c —
// порог «корабль остановился» в ветке экстренной остановки.
const double RESCUE_DOCK_DISTANCE = 0.01;
const double RESCUE_MATCH_SPEED = 0.0001;
// Как часто диспетчер опрашивает маяки. Ход света и полёт спасателя меряются
// ГОДАМИ, поэтому опрашивать тысячу бортов сто раз в год незачем: четверть года
// — это уже мельче, чем самое короткое ожидание, и вчетверо реже такта 0.01.
const double DISTRESS_POLL_YEARS = 0.25;
...
// (§50) Спасение как механика: чем оно кончилось и сколько ждали.
long long strandStatBeacons();       // поднято сигналов бедствия
long long strandStatLiveRescues();   // вытащено ЖИВЫМ бортом (дошёл и состыковался)
long long strandStatStateRescues();  // вытащено казённым спасателем
long long strandStatLastResort();    // сработал TOW_WAIT_YEARS — физика не справилась
long long strandStatLoots();         // на сигнал первым пришёл чужой и забрал груз
double strandStatWaitAverage();      // среднее ожидание спасённого, годы
double strandStatWaitWorst();        // худшее то же
double strandStatLightAverage();     // из него — только ход света до спасателя, годы
double strandStatBountyPaid();       // списано с казны за спасения (награда + топливо)
double strandStatBountyCash();       // из этого дошло до карманов спасателей
...
// (§50) МАЯК БЕДСТВИЯ.
//
// Хранит не «кто уже знает», а ТОЧКУ и МОМЕНТ подъёма: сигнал идёт со светом
// (§16), поэтому «услышал» — это одно сравнение `time >= raised + расстояние`,
// а не список рассылки. В скоплении единицы расстояния — световые годы, времени
// — годы, так что скорость света в этой формуле равна единице и отдельной
// константы не требует (так же считается вся очередь `pendingSignals`).
//
// ⚠️ Точка МЁРТВАЯ, а борт живой: он уходит баллистически с той скоростью, с
// какой умер двигатель. Поэтому маяк несёт ещё и вектор скорости — по нему
// спасатель, долетев до точки сигнала, восстанавливает НАСТОЯЩЕЕ положение
// борта (решение пользователя, §48.8 п.2). Догонит или нет — его забота: гасить
// скорость дрейфа некому.
struct DistressBeacon {
    int agent = -1;              // кто зовёт
    double x = 0.0, y = 0.0, z = 0.0;      // где сигнал поднялся
    double vx = 0.0, vy = 0.0, vz = 0.0;   // с какой скоростью борт уходил
    double raisedTime = -1.0;    // когда поднялся — от него и считается ход света
    int responder = -1;          // борт, идущий на перехват (−1 — никто не идёт)
    // Казна уже строила катер под этот маяк. Флаг НИЧЕГО НЕ ЗАПИРАЕТ: строить
    // ли ещё один, решает оценка встречи и потолок населения (см.
    // `dispatchStateCutter`). Он нужен диагностике — «палата знает о беде и
    // пробовала» отличается от «палата ещё не услышала».
    bool stateSent = false;
    bool looted = false;         // на сигнал уже приходили чужие и вычистили трюм
};
...
public:
    ...
    // (§50) Поднятые сигналы бедствия. Список короткий по природе: замер §48.9
    // дал одно попадание в беду на 13 лет в полном мире, и маяк снимается, как
    // только борт вытащили.
    std::vector<DistressBeacon> distress;
    ...
    void processSignals();
    // (§50) Спасение из дрейфа. `updateDistress` — диспетчер: кто услышал маяк
    // и кто идёт на перехват. Ходит РЕЖЕ такта: ход света меряется годами, и
    // опрашивать тысячу бортов сто раз в год незачем (замер такта — §50.5).
    void updateDistress(double dt);
    // Кого зовёт игрок и куда лететь: маяки, чей свет ДОШЁЛ до его борта.
    // Возвращает индексы в `distress`, свежие первыми.
    std::vector<int> audibleDistress(int agentIndex) const;
    // Взяться за спасение: игрок летит к точке сигнала, а не к звезде. Отказ,
    // если борт уже в пути, если маяк не слышен или если его крейсерская ниже
    // скорости дрейфа — догонять нечем (§48.6).
    bool commandAgentToDistress(int agentIndex, int beaconIndex);
