...
//
// Планка первого тысячелетия — 10 000, а не 1 000: тысяча кредитов тарифа за
// ТЫСЯЧУ ЛЕТ закрывалась дюжиной рейсов и лежала мёртвым грузом три четверти
// периода — обязательство, которого не чувствуешь, ничему не учит. Десять тысяч
// держат игрока в маршруте весь период, но всё ещё берутся одним бортом (замер —
// в `make balance`, «квота закрывается за разумное число рейсов»).
const double LICENCE_PERIOD_YEARS = 1000.0;      // релятивистская корректировка рынка
const double LICENCE_QUOTA_BASE = 10000.0;       // квота первого тысячелетия
...
// роста флота.
//
// ⚠️ Цена ВЫВОДИТСЯ из квоты (`licenceQuotaBase * K * licenceCount`), поэтому при
// подъёме квоты 1 000 → 10 000 множитель опущен 1000 → 100: миллион за второй борт
// — это design point, а не побочный продукт. Пропускная способность игрока от
// размера квоты не изменилась, значит и цена борта в кредитах меняться не должна.
...
const double SYSTEM_PRICE_POP_REF = 20000.0;     // население, удваивающее цену
...
const double SYSTEM_PRICE_TURNOVER_REF = 2200.0; // годовой выпуск недр (Cr/год), удваивающий цену
