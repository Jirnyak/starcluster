...
- Asteroids are lit ray-sphere boulders only when they are close enough to fill more
  ...
  *rate* above) and by the host system's mining richness, a field that previously drove only
  ...
  so the generation stream stays bit-for-bit identical. Still open: the per-unit *market
  price* of ore still comes only from the element mapping, not a class premium at the till.
