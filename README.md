...
- **lit asteroid boulders**: nearby belt rocks use the same ray-sphere technique with a
  ...
  ore, carbon the poorest — so the glint doubles as a gameplay beacon for the good ore;
...
- Asteroids are lit ray-sphere boulders only when they are close enough to fill more
  ...
  composition (ice/carbon/metal/silicate) via a shared classifier that now drives both
  appearance (palette + specular glint) and mining extraction rate (metal is richest,
  carbon poorest), so the visible glint reads as a gameplay beacon. Still open: the
  *market value* of what you mine comes only from the element mapping, not a class
  premium, and belt spawn rates are composition-blind — every class is equally common.
