...
| `soak_test.cpp` | Headless long-run soak harness for the local simulation (`make soak`): 120k deterministic frames under ASan/UBSan plus isolated probes for the player-death path, the player-kill macro write-back (§5.13.14), the mirror-trader sell write-back (§5.13.18), and the NPC-vs-NPC macro write-back (§5.13.26, `npcWriteBack`), each on a fresh `Game` so it never perturbs the main-loop baseline. |
...
- Local NPC traffic is now goal-driven (errands to markets, timed arrivals and departures)
  ...
  **sell** on docking — a real deterministic sale that moves cargo, credits, market prices, and
  ...
  real time, so bounties are emergent as well as pre-generated (§5.13.26).
