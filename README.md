...
still, but burns fuel proportionally, so a rig buys time rather than margin. The
rocks themselves regenerate whenever local flight is re-entered, but the *reachable*
belt of a system does not: what one ship can scrape near its entry corridor is
worked over as it is taken, halving the yield after roughly a holdful and drifting
back over decades on the same slow valve as a system's bounty budget. The system's
own deposits are untouched by this - industry there moves thousands of tons a year,
and a lone captain cannot dent them. So mining stays viable and becomes nomadic
rather than a fixed farm. Volatiles mined out of ice
...
- Startup generates 8,192 systems and takes several seconds with no loading
  screen.
- A save now carries the future, not just the moment (save version 21). Measured
  on the full world (8,192 systems, 150 years in, 40 years out, game timestep):
  the clearing house treasury reads 3.90836e9 in the original and 3.90836e9 in
  the reloaded copy, identical to the digit. What used to be missing was not
  numbers but *clocks*: the market sweep phase, the accumulated `strain` and
  `serviceCostAvg`, and `seededScale` - the memory of the last economy rescale.
  ⚠️ The 1.2% divergence reported here before was partly an artefact of the probe
  itself: `rng` is a process global, so running two worlds interleaved in one
  process makes them draw from a shared stream and diverge no matter how correct
  the save is. Run one copy, restore the generator, run the other. The underlying
  defect was real too - the corrected probe still showed 4,243 differences on the
  old code against 0 now. `save_probe.cpp` and `save_diff.cpp` cover this, and
  `testSaveCarriesTheFuture` guards it in the harness.
- A game-year of simulation costs about 165 ms at 8,192 systems and roughly a
  ...
  falls behind instead. Most of what remains is the market substitution model and
  the RNG, not routing. ⚠️ There is a *procedure*, not a number: `tick_probe`,
  8,192 systems, 200 years, seed 20260814, game timestep 0.01, three repeats,
  median, machine otherwise idle. On this commit that gives 165.1 / 165.4 / 165.7
...
- Powers now buy trading licences and commission hulls from their treasuries, so their
  ...
  number of agents. Measured at full scale: 1,016 agents and 165 ms per game year
  on 8,192 systems, by the procedure above.
- A chromocore now lands on the stat you *practised* rather than a 1-in-7 roll:
  surveying feeds intellect, trading charisma, mining and salvage materials,
  fighting tactics, flying kinematics, radio sources sensors, anomalies luck. What
  did **not** change is the tempo - still 2 cores in a measured sixty-run career,
