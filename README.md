...
- **the law you crossed hunts you back**: once your standing with a faction sinks to Enemy or Hostile
  ...
  *more* uptime, never less; zero RNG, no new field, `factionRelation` bounds-guarded. A two-phase
  ...
  (§5.13.42);
- **the cockpit tells you when the law is hunting *you***: §5.13.42's standing-triggered manhunt is now
  legible from the cockpit — a patrol that has locked *you* with no provocation flies a crimson `HUNTING
  YOU` label over its hull and the fastest, brightest pulse-rings in the HUD (rate 7.4, distinct from the
  mooring/escort/wanted/SOS cadences), tinted by your standing tier; off-screen, the same hunter draws a
  standing-colored edge arrow. So the "why am I being shot at without firing?" moment from §5.13.42 finally
  has an answer *on screen*. The key restraint: **zero new fields and zero new sim behavior** — the render
  reads the pre-existing but dead `LocalCraft::aiTarget`, which the sim now populates read-only right after
  it sets `aiState` (`LOCAL_TARGET_PLAYER` when the target is the player, else the craft index, else −1).
  Because no sim decision reads `aiTarget`, behavior is untouched and the soak stays bit-for-bit identical.
  Both the on-screen cue and the off-screen edge marker call one shared `huntsPlayerByStanding` predicate
  (`CK_PATROL && aiState==1 && aiTarget==player && !hostile && hullHP>0`) so they cannot drift; `!hostile`
  excludes a provoked patrol (its hostility is self-evident) and `aiState==1` excludes a fleeing one (a
  low-hull hunter breaks off → `aiState=2` → `aiTarget=−1` → cue goes dark, which is honest). The trigger
  is the `aiTarget` field — *who the patrol is actually chasing* — not merely CK_PATROL + Enemy standing,
  proven by a `shot_hunted_none` twin where the same Enemy patrol chasing a *pirate* shows no cue. Three
  screenshots pin it (`shot_hunted_standing` cue-on, `shot_hunted_none` cue-off, `shot_hunted_edge`
  off-screen arrow) and a `patrolTargetSignal` soak probe reads `aiTarget` directly (player-sentinel at
  Enemy standing, a pirate index when one is nearer). An adversarial review returned all-confirmed; its
  findings drove three robustness improvements — the shared predicate helper, the off-screen coverage
  shot, and a folded-in `hullHP>0` guard (§5.13.43);
- **the deeper you're hated, the faster the law runs you down**: a patrol hunting *you* by standing
  (§5.13.42) now flies faster the deeper its faction's rep with you sinks — the mirror of the §5.13.34
  manhunt, but keyed to your *personal* standing instead of a pirate's bounty. A file-static
  `vendettaSpeedMult` multiplies the patrol's movement speed cap by `1 + VENDETTA_SPEED_GAIN ·
  clamp((−48 − rep)/80, 0, 1)`, so rep −48 (the Hostile threshold where §5.13.42's override switches on)
  gives ×1.0 and rep −128 (the floor) gives ×1.6. This is the first time the sim *consumes* the
  `aiTarget` field §5.13.43 began populating read-only — it graduates from a render-only signal to a
  live sim input (the movement loop is a separate pass that runs after the AI loop, so `aiTarget` is
  already committed). The `GAIN` (0.6) is deliberately above the manhunt's (0.5): a personal vendetta is
  more relentless than a professional bounty hunt. The two multipliers are disjoint by construction — a
  player-hunt clears `defending` (which `manhuntSpeedMult` gates on) and an explicit `aiTarget==player`
  guard was added to `manhuntSpeedMult` — so at most one speed bonus applies per craft. Strictly additive
  (design filter §0.2-G): rep above −48 gives ×1.0 (the prior behavior), only the *patrol's* speed changes
  (target choice, damage and pirates are untouched), and a faster law chasing you means *more* combat, not
  less. Zero RNG, no new field; the `CK_PATROL` branch never runs in the soak, so the baseline is
  bit-for-bit unchanged. A two-phase `vendettaScalesWithStanding` probe pins it — a floor-rep (−128) patrol
  saturates at maxSpeed·1.6 while a threshold-rep (−48) patrol *still* hunts the player but moves at plain
  maxSpeed (heat 0), proving the bonus scales with standing depth, not with the mere fact of being hunted
  (§5.13.44);
- **the cockpit shows how much you're hated**: the crimson "HUNTING YOU" cue (§5.13.43) now anneals with the
  same vendetta `heat` that drives §5.13.44's chase speed — the deeper the hunting patrol's faction rep with
  you has sunk, the hotter its tint (warming toward white-hot), the faster its ring pulse, and the brighter
  its base glow. A file-static `vendettaHuntColor` re-derives `heat = clamp((−48 − rep)/80, 0, 1)` draw-side,
  bit-for-bit identical to §5.13.44's `vendettaSpeedMult`, so the visual dial and the speed dial can never
  disagree. Both the on-screen cue and the off-screen edge marker read that one helper, so their tints can't
  drift apart. Purely draw-only — it reads live `factionRelation` each frame, adds no field and no RNG, so
  the soak baseline is bit-for-bit unchanged; and at rep −48 (heat 0) every heat term vanishes, reproducing
  the §5.13.43 cue exactly (§5.13.45);
- **hostile patrols hunt you as a pack**: §5.13.42 lets a patrol of a faction you stand Enemy/Hostile with
  target *you* — but only through the reused `pd2 < atkBest` gate, so a patrol with a closer *wanted* pirate
  chases the pirate and leaves you for later. §5.13.46 adds a coordination tier (last in the `CK_PATROL`
  branch): if a second live hostile patrol within `PATROL_DISTRESS_R` also stands Enemy/Hostile with you and
  can see you, *both* drop their pirates and converge on you — the player-keyed mirror of the pirate rally
  (§5.13.30) and the patrol backup (§5.13.32). Deep hatred stops being a passive speed penalty (§5.13.44) and
  becomes coordination: the more enemies around you, the higher you rise on their target list. The "is a
  neighbor disposed to hunt you" test is re-derived from live `kind`/`faction`/`hullHP`/positions + standing,
  never from the neighbor's committed `aiTarget`/`defending`, so two mutually-disposed patrols each commit on
  the other's *disposition* (not its commit) — order-independent, no deadlock, both flip in one frame. It
  sets `atkBest = pd2` alongside `atkPlayer` (exactly as §5.13.42 does) so the fire-range check reads the
  player distance, not the abandoned pirate's. Strictly additive (design filter §0.2-G): the tier only moves
  a patrol *off* a pirate and *onto* you (never the reverse), so pirates gain uptime and you gain combat; with
  no pair, rep above −48, or you outside awareness it falls back to the prior §5.13.42 behavior. A free render
  bonus falls out — the "HUNTING YOU" cue (§5.13.43) and vendetta heat (§5.13.45) now light *every* pack
  member, since all carry `aiTarget==player`. Zero RNG, no new field (reuses
  `atkBest`/`atkPlayer`/`atkCraft`/`PATROL_DISTRESS_R`); the `CK_PATROL` branch never runs in the soak, so the
  baseline is bit-for-bit unchanged, and an isolated two-phase `lawPackHuntsPlayer` probe pins it — two Enemy
  patrols each with a closer wanted pirate flip both onto the player, while a lone patrol keeps chasing its
  pirate (proving a *pair* is required) (§5.13.46);
- **you can see the pack close in**: the coordinated hunt from §5.13.46 is now drawn with two purely-cosmetic
  cues. A screen-fixed top-center **`PACK HUNT X<N>`** banner (`PACK HUNT X3` for three hunters) lights up
  *only* when two or more patrols are hunting you by standing — the definition of a "pack" (a lone hunter is
  already announced by the per-craft "HUNTING YOU" cue), tinted by the *hottest* vendetta heat among them (the
  worst enemy sets the tone). And on the radar, a thread runs from *every* standing-hunter's blip to the center
  (= you) in that hunter's vendetta tint — the net drawing closed. The threads also fix a radar blind spot: an
  unprovoked hunter carries `!hostile`, so §5.13.27 painted its blip the same dim grey as neutral traffic; the
  colored thread now marks it. Both cues re-derive "who is a hunter" from the same live-field predicate
  (`huntsPlayerByStanding`) as the per-craft cue and tint with the same helper (`vendettaHuntColor`), so banner,
  threads, world label, and blip cannot drift apart; `radarXY` maps out-of-range contacts exactly onto the radar
  rim, so threads never streak off the panel. Draw-only: zero new fields, zero RNG, the simulation is untouched
  (§0.2-G trivially satisfied) and the soak baseline is bit-for-bit unchanged; a new `shot_pack_hunt` screenshot
  (three hunters — two white-hot at rep −128, one warmer at −70) proves the banner and all three threads render,
  with the N=1 and N=0 negatives covered for free by the existing `shot_hunted_standing`/`shot_hunted_none`
  shots (§5.13.47);
- **killing a cop costs you double**: gunning down a police/patrol ship now hits your standing with
  that ship's faction *twice as hard* as killing an ordinary civilian trader (`−16` vs the generic
  `−8`), and — unlike the old civilian reprisal — it applies **even if that patrol was already
  hostile**, closing a gap where murdering a patrol you had already provoked cost you nothing. The
  heavier reputation hit feeds straight into the existing law machinery: it pushes you toward the
  Enemy/Hostile threshold (rep ≤ −48) sooner, so patrols start hunting you on sight (§5.13.42), run
  you down faster the deeper you sink (§5.13.44), and coordinate into a pack (§5.13.46) — all of it
  already visualized by the `PACK HUNT` banner and radar threads (§5.13.47), so the escalation needs
  no new cue: a `COP KILLED -REP` toast marks the moment and the rest is already on screen. The
  patrol-specific branch *replaces* (does not stack with) the generic reprisal for a patrol, so a
  single kill is penalized once; non-patrol kills take the byte-identical old `−8` path. Strictly
  additive to combat and the invariant is *strengthened* — deeper law enmity means more law-combat
  aimed at you, and patrols peeled off pirates onto you leave pirates more uptime (§0.2-G); zero RNG,
  no new field. The headless soak spawns no patrols, so the branch is a no-op there and the 120k
  baseline stays bit-for-bit unchanged; a three-scene `copKillDeeperReprisal` probe pins it — a
  non-hostile patrol and an already-hostile patrol both cost `−16` while a control trader still costs
  `−8` (§5.13.48);
