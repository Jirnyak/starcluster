...
int updateLocalScene(Game& game, LocalScene& scene, const LocalInput& in, double dtReal) {
    ...
    for (size_t i = 0; i < scene.craft.size(); ++i) {
        ...
        if (c.kind == CK_PIRATE) {
            ...
            // (§5.13.42) ЗАКОН, КОТОРЫЙ ТЫ ПЕРЕШЁЛ, ОХОТИТСЯ НА ТЕБЯ. Патруль берёт игрока целью не только когда
            //   СПРОВОЦИРОВАН (c.hostile — игрок выстрелил первым / сбил борт его фракции, стр.1042-1051), но и когда
            //   игрок УЖЕ стоит с фракцией патруля на тире Enemy/Hostile (реп ≤ −48) — ровно тот порог, что §5.13.40
            //   карает −6 rep за co-op-добивание, а §5.13.41 красит вектор погони в красный. Так «месть фракции»
            //   получает зубы: накопил вражду (репрайзы §5.13.40 / −8 за сбитый борт фракции / макро-действия) — и
            //   патрули этой фракции нападают БЕЗ свежей провокации. Тир — из ТОГО ЖЕ классификатора, что панель
            //   §5.13.37 / прицел §5.13.39 / вектор §5.13.41 ⇒ читалка и поведение согласованы ПО ПОСТРОЕНИЮ. Реюз
            //   СУЩЕСТВУЮЩЕГО гейта предпочтения игрока (pd2 < atkBest): патруль бросит дичь ради игрока ТОЛЬКО если
            //   игрок БЛИЖЕ — как уже делает спровоцированный патруль (никакого нового тир-приоритета; охота §5.13.28
            //   в обычном случае, когда пират ближе, цела). Комбат-инвариант §0.2-G: пиратов не трогаем — если патруль
            //   отвлёкся на игрока, у пирата аптайма БОЛЬШЕ, не меньше ⇒ «уменьшает урон/агрессию/аптайм пирата?» —
            //   НЕТ. Ноль RNG, без нового поля; classify пропускаем, если уже hostile (OR и так истинен); factionRelation
            //   защищён по границам (мусор/своя фракция ⇒ Neutral ⇒ false). В соаке ветка не исполняется в
            //   ОСНОВНОМ цикле: 18 воен-агентов init() стоят гарнизонами на controlledStars (разбросаны по
            //   кластеру-1200: 78/792/1073/…), disjoint фикс-маршруту soak {-1,0,1,2,3,5,7,11,23,99}; co-location
            //   гейт currentStar==звезда (localgen.cpp:438) ⇒ patrols=0 на всех 10 ⇒ 120k-бейзлайн побитово цел.
            //   Пробник lawHuntsHostile инъектит синтетический CK_PATROL ⇒ АКТИВНОЕ покрытие ветки (эмпир. проверено).
            bool factionHuntsPlayer = false;
            if (!c.hostile && c.faction >= 0 && game.playerFaction >= 0) {
                FactionRelationTension st =
                    classifyFactionRelationTension(game.factionRelation(game.playerFaction, c.faction));
                factionHuntsPlayer = (st == FactionRelationTension::Enemy ||
                                      st == FactionRelationTension::Hostile);
            }
