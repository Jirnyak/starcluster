...
namespace LocalCfg {
    ...
    // (§22) РАЗРУШЕНИЕ выработанного астероида. Пустая глыба раньше висела в поясе
    // неотличимо от полной — пояс превращался в кладбище декораций, по которому игрок
    // заново облетал уже выжатые камни. Теперь последняя тонна раскалывает камень:
    // время — РЕАЛЬНЫЕ секунды (как fxClock/hitFlash), поэтому анимация идёт одинаково
    // и под warp. Дольше секунды держать нечего: это не смерть корабля, а расход.
    constexpr double ROCK_BREAK_TIME  = 1.10;   // от первой трещины до исчезновения, реальные сек
    constexpr int    ROCK_SHARD_COUNT = 10;     // на сколько кусков разлетается глыба (шейдер §22.2)
    constexpr double ROCK_SHARD_SPREAD = 1.35;  // насколько далеко (в радиусах) уходит самый резвый кусок
...
struct LocalRock {
    ...
    double grade = 1.0;
    // (§22) Разрушение выработанной глыбы. `breakT` — РЕАЛЬНЫЕ секунды с момента, когда
    // из камня вышла последняя тонна (0 = камень цел); дойдя до ROCK_BREAK_TIME, камень
    // становится `gone` и выбывает из сцены целиком: не рисуется, не ищется буром, не
    // считается подсказкой. Индексы в векторе при этом НЕ едут — `scene.miningRock`/
    // `minePrompt` хранят именно индексы, поэтому камень гасится флагом, а не erase().
    double breakT = 0.0;
    bool   gone   = false;
};
