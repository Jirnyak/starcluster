...
const Entry EXACT[] = {
    ...
    // ⚠️ (§46) ЭТИ СЕМЬ — В EXACT, А НЕ В WORDS, И ЭТО НЕ ВКУСОВЩИНА.
    // Пословная сборка не берёт ключ, который КОНЧАЕТСЯ знаком препинания
    // (правило записано над таблицей WORDS), а у всех семи на конце скобка
    // с горячей клавишей. Лежа в WORDS, они молча переводились по словам, и
    // русский игрок читал в панели целей «ГОДНОСТЬ A CONTAINMENT ЯЧЕЙКА (Y)»
    // и «КУПИТЬ A СИСТЕМА (C)» — семь ступеней лестницы из четырнадцати.
    {"MEET THE QUOTA - OR SETTLE IT (E)", "ЗАКРЫТЬ КВОТУ ИЛИ ОТКУПИТЬСЯ (E)"},
    {"BUY A LICENCE (E), THEN A HULL (U)", "ЛИЦЕНЗИЯ (E), ПОТОМ КОРПУС (U)"},
...
const Entry WORDS[] = {
    ...
    {"THREATS", "УГРОЗЫ"},
    {"AGE", "ВОЗР"},
    // Столбцы сводки биржи (§46): цена дороги и срок были в ней не показаны.
    {"YRS", "ЛЕТ"},
    // ⚠️ Единственный текст, который видит ЗАПЕРТЫЙ игрок (§46). Собран одним
    // оборотом нарочно: точным ключом его не взять (внутри число), а пословно
    // он рассыпался в «ПРОДАТЬ ТРЮМ, ЭКЗОТИКА OR АКЦИИ КУДА WORK ВЫКЛ».
    {"TRADING FROZEN - PAY IT WITH F2 OR SELL CARGO, EXOTICS OR SHARES TO WORK OFF",
     "ТОРГОВЛЯ ЗАМОРОЖЕНА - ОТКУПИТЕСЬ ПО F2 ИЛИ ОТРАБОТАЙТЕ ПРОДАЖЕЙ ТРЮМА, ЭКЗОТИКИ ИЛИ АКЦИЙ"},
    // Строка заказа (§46): дорога в кредитах и плата за год полёта.
    {"ROAD", "ДОРОГА"},
    {"NET", "ЧИСТЫМИ"},
    {"PAID", "ОПЛАЧЕНА"},
    ...
    {"MINED", "ДОБЫТО"},
    // Характеристики корпуса на верфи (§46). Раньше их шесть штук стояли
    // кодами — `CG HW LW AR U`, — и расшифровки не было нигде. Слова короткие:
    // строка целиком должна влезть в 390 px до кнопок покупки.
    {"HEAVY", "ТЯЖ"},
    {"LIGHT", "ЛЁГК"},
    {"ARMOR", "БРОНЯ"},
    {"SLOTS", "СЛОТЫ"},
    // Кнопки покупки корпуса. `UPG` — это «улучшить»: пересесть, доплатив
    // разницу; `NEW` — второй борт по свободной лицензии.
    {"UPG", "УЛУЧШ"},
    // ⚠️ Ключ «NEW» безопасен ТОЛЬКО потому, что совпадение жадное: «NEW GAME»
    // в меню оболочки длиннее и выигрывает. Другого одиночного «NEW» в игре нет
    // (проверено grep'ом по всем .cpp) — добавишь, проверь снова.
    {"NEW", "НОВЫЙ"},
    // Карточка управления: слова, которых в словаре ещё не было.
    {"ADVISOR", "СОВЕТ"},
    {"EXOTICS AND FORGE", "ЭКЗОТИКА И КУЗНИЦА"},
    {"FLY", "ПОЛЁТ"},
    {"ROLL", "КРЕН"},
    {"PITCH", "ТАНГАЖ"},
    {"FIRE", "ОГОНЬ"},
