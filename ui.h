...
// Лестница целей (§46). Наружу вынесена РАДИ ХАРНЕСА: панель 248 px рисует
// текст с отступом 34, то есть держит 208 px, а две ступени из четырнадцати за
// эту рамку вылезали — и `make uishots` их не ловил, потому что на нулевом году
// они ещё за окном лестницы. Копировать список в харнес нельзя: это ровно тот
// «список, который надо помнить», на котором проект обжёгся в §30 и §45.
struct ObjectiveStep {
    const char* text;
    bool done;
    bool urgent;   // «сделай сейчас» — рисуется под лестницей, а не в ней
};
