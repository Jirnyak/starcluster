...
void testGoButtonGoesToTopWindow() {
    ...
    // Баки под пробку: тест кликов не про дозаправку.
    {
        ...
        // (§48) Баки заливаются ПОД СВОЮ ПРОБКУ, а не до миллиона единиц.
        //
        // ⚠️ Здесь ёмкости раздувались до 1e9 и заливались 1e6 тория с
        // водородом — «убрать законные причины отказа». С §48 топливо в бункере
        // честно считается мёртвой массой манёвра, и миллион единиц тория
        // делает корабль НЕЛЕТАЮЩИМ по-настоящему: рабочего тела на разгон
        // такой горы не хватит ни при каком запасе. Полные штатные баки дают
        // ровно то, ради чего блок писался, и не спорят с физикой.
        shipEmergencyPrime(s, 1.0);
    }
