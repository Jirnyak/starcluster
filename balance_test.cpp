...
void testBoardHonest() {
    ...
    for (unsigned seed : seeds) {
        Game g; buildWorld(g, seed, 80);
        // (§48) ОБОРОТНЫЙ КАПИТАЛ ВЫДАЁТСЯ ЯВНО — как во всех остальных местах,
        // где зовётся `executeTopDeal`.
        //
        // ⚠️ Здесь проверка шла на СТАРТОВОМ кошельке (100 Cr). Пока маршрутная
        // оценка недосчитывала дорогу вдвое (§48), лучшая сделка выходила
        // прибыльной, и проверке было что сверять. С честной ценой дороги на
        // 100 Cr прибыльных сделок нет НИ ОДНОЙ из сорока ни в одном из четырёх
        // миров — и это замысел (§ стартовый капитал: «игрок начинает нищим»,
        // первая задача — на что вообще купить груз). Проверка же отсеивает
        // убыточные сделки, оставалась с нулём проверок и молча не проверяла
        // НИЧЕГО. Утверждение — прогноз сходится с исполнением — не тронуто.
        g.agents[g.playerAgent].money = 3600.0;
        ArbitrageDeal d; bool ok = false;
...
void testInsightBurnsReactorFuel() {
    ...
    // (§48) Бункер доливается ДО ПРОБКИ явно. Проверка меряет «процент ПОЛНОГО
    // бака» и сама об этом пишет ниже — но с §48 корпус выходит с верфи с
    // рабочим запасом (`FUEL_START_SHARE`), а не полным, и премисса молча
    // переставала выполняться: доля от остатка выходила вчетверо больше.
    shipEmergencyPrime(ps, 1.0);
...
void testJobBondBurnsOnFailure() {
    ...
    char buf[240];
    std::snprintf(buf, sizeof(buf),
        "залог %.4f: кошелёк %.4f -> %.4f (взятие, списано %.4f) -> %.4f (провал %s); на нулевом тире залог %.4f",
        bond, before, afterTake, before - afterTake, afterFail, failed ? "да" : "НЕТ", zeroTierBond);
    ...
    // ⚠️ (§57) Нижний конец сверяется НЕЗНАЧИМОСТЬЮ, а не точным нулём. Тир
    // заказа стал НЕПРЕРЫВНЫМ — он описывает реальный размер партии, а не
    // ступень жребия, — поэтому у заказа на волос крупнее базового и залог на
    // волос больше нуля (замер: 1e-14 Cr, печатается как 0.0000). Утверждение
    // проверки — «новичку доска не заперта залогом», и оно цело: стартовый
    // кошелёк 100 Cr, а барьером тут и кредит не назовёшь. Требовать бит-в-бит
    // ноль значило требовать, чтобы тир был ступенчатым, — то есть проверять
    // прежнюю реализацию, а не смысл.
    check(taken && failed && bond > 0.0 && zeroTierBond < 1.0 &&
