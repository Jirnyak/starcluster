...
public:
    ...
    // (§48) Умолчание — ПОЛОВИНА потолка, а не полный ход.
    //
    // ⚠️ Полный ход — самое дорогое деление этой ручки, и до §48 он был
    // умолчанием только потому, что маршрутная оценка не показывала его цену.
    // Замер после починки оценки (плечо 7 ly, кошелёк 400 Cr): на полном ходу
    // дорога съедает 105% выручки и 7 стартов из 12 убыточны, на половине —
    // 52% и 3 из 12. Прежний баланс мира (49% до §48) воспроизводится ровно
    // здесь, на 0.5, и по Cr за год полёта максимум стоит там же: −11 Cr/год
    // на полном ходу против +61 на половинном. Форсаж остался — но теперь он
    // осознанная трата за скорость, а не молчаливое умолчание.
    double cruiseFraction = 0.5;

    std::vector<Resource> cargo;
    double cargoCapacity = 100.0; // Максимальный груз (масса)

    // (§48, ВРЕМЕННЫЙ ЗАМЕР) Что маршрутный расчёт обещал на плечо и что реально
    // залили при вылете. Нужны, чтобы сверить оценку с расходом. Не сохраняются.
    double dbgPlannedProp = 0.0;
    double dbgLoadedProp = 0.0;
    double dbgLegDistance = 0.0;
    double dbgPlannedRapidity = 0.0;   // бюджет быстроты, заложенный в оценку плеча
    double dbgUsedRapidity = 0.0;      // быстрота, реально выданная двигателем
    double dbgHonestNeed = 0.0;        // расход по ФАКТИЧЕСКОЙ массе борта (формула самого полёта)
    ...
    // Ступени переоснастки живут НА КОРПУСЕ, а не в партии.
    //
    // ⚠️ Сначала они лежали в `Game`, и это давало сразу две ошибки: наварив
    // восемь слоёв на первый борт и пересев на второй, игрок получал те же
    // слои и там (оплачено один раз, применено дважды), а после загрузки
    // ёмкость ячейки восстанавливалась только у пилотируемого борта — экзотика
    // на втором оказывалась в отсеке нулевого объёма. Переоснастка — это
    // сваренный металл на конкретном корпусе, и место ей здесь.
...
// (§48) Доля бункера, с которой корпус выходит с верфи. Топливо не улетает в
// сопло — оно перегорает в золу и остаётся на борту, то есть везётся как груз.
// Полный бункер вчетверо тяжелее корпуса, и оплачивает его рабочее тело.
const double FUEL_START_SHARE = 0.25;
...
// Масса ИНЕРТНОЙ экзотики: нейтрониум и конденсат. Антивещество сюда не
// входит — оно едет в бункере и уже посчитано в топливной смеси. Отдельная
// функция нужна планировщику маршрута: без неё полный отсек нейтрониума (до
// 1440 единиц массы при сухой массе корпуса в 5..400) был для расчёта топлива
// НЕВИДИМ, маршрут считался по заниженной массе, а сжигался по настоящей —
// и корабль вставал посреди перелёта ровно на том рейсе, ради которого ячейку
// и покупали.
