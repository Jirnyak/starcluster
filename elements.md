...
Три величины, из которых растёт вся двигательная модель (`ship.md`). Все
непрерывны и считаются для всех 118 элементов одинаково, без спецкейсов.

Удельная ядерная энергия — расстояние до пика кривой связи. Одна формула на
синтез и деление, ноль ровно на железе:

```text
specificEnergy = max(0, bindingPeak - bindingPerNucleon)   // МэВ/нуклон
```

Плотность. Молярный объём по таблице довольно ПЛОСКИЙ (5..70 см³/моль),
поэтому разброс плотностей почти целиком делает атомная масса: водород рыхлый
не потому, что атом большой, а потому что весит единицу. Объём растёт с
номером оболочки и стягивается металлической связью d-электронов (максимум у
полузаполненной оболочки — оттого осмий и вольфрам плотнее меди):

```text
dBond       = 0.4*dFill^0.6 + 0.6 * 4*dFill*(1-dFill)
fBond       = 4*fFill*(1-fFill)
molarVolume = period^0.90 * (1 + 1.6*nobleStability) / (1 + 2.8*dBond + 2.0*fBond)
density     = 0.169 * atomicMass / molarVolume
```

Коэффициенты подогнаны по 41 реальной плотности, типичная ошибка 1.5x,
худшая 2.6x (бор). Порядок верный: наверх выходят осмий, платина, ртуть и
актиниды, вниз — водород и благородные газы.

Лёгкость ионизации (нужна электрическим двигателям). Внешний электрон тем
дешевле оторвать, чем он дальше от ядра и чем слабее ядро его держит;
замкнутая оболочка держит намертво:

```text
ionizationEase = clamp01(0.10 + 0.55*(1-electronegativity) + 0.30*(period-1)/6
                         - 0.85*nobleStability - 0.22*dFill - 0.30*fFill)
```

Наверх выходят франций, радий и цезий, вниз — благородные газы и галогены,
как в жизни.

ВАЖНО: `specificEnergy` говорит, сколько энергии в ядре ЕСТЬ, но ничего не
говорит о том, можно ли её достать. Достижимость — это
`max(fusionFuelTrait, fissionFuelTrait)` с кулоновским барьером и окном
делимости. Выбирать топливо по одной энергии нельзя: наверх вылезут
сверхтяжёлые, которые живут секунды.
