...
public:
    ...
    // тяга есть.
    // (§50) Теперь СОХРАНЯЕТСЯ (сейв 20). Раньше не сохранялся сознательно —
    // ждать было нечего, помощь приходила по таймеру. С маяком у беды появился
    // возраст: сигнал ушёл в такой-то год, спасатель уже летит. Обнулять это
    // при загрузке значило бы воскрешать погашенные маяки и звать помощь
    // дважды. Возраст самого маяка лежит в `DistressBeacon::raisedTime`.
    double driftYears = 0.0;
    // (§50) Спасательный рейс: индекс маяка в `Game::distress`, за который борт
    // взялся (−1 — не спасает никого). Пока `rescueKnows` не поднят, борт летит
    // к ТОЧКЕ СИГНАЛА и настоящего положения не знает; долетев — считает его по
    // вектору скорости из маяка и переходит на перехват (§48.8 п.2).
    int rescueTarget = -1;
    bool rescueKnows = false;
    // Куда возвращаться, бросив дело: спасатель отвлекается с полпути, и без
    // этого он после спасения не помнил бы, куда вёз груз.
    int rescueResumeStar = -1;
    int targetFaction = -1;
