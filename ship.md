...
- Save-версия 10; старые сейвы отвергаются.
...
Ручка `throttle` (0..1) выбирает точку внутри физически допустимой полосы:

```text
0.0  минимальная скорость истечения: рабочего тела много, топлива мало,
     корабль тяжелее и медленнее
0.5  ценовой оптимум СРЕДИ ВЛЕЗАЮЩИХ В БАКИ режимов (значение по умолчанию)
1.0  скорость истечения у потолка: рабочего тела минимум, топлива максимум,
     корабль легче и разгоняется быстрее
```

Интерполяция логарифмическая, потому что и стоимость, и массовое число зависят
от скорости истечения экспоненциально: в линейной шкале середина ощущалась бы
как «почти форсаж».

Важно, что крайне левое положение — НЕ самое дешёвое: минимум расхода топлива
лежит около 0.15, а дальше влево топливо снова растёт, потому что рабочего тела
приходится швырять слишком много. Поэтому ось подписана не «эконом/форсаж», а
честно — ЧЕМ платим: `PROP` слева, `FUEL` справа.
