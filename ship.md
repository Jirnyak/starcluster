...
- Save-версия 14; старые сейвы отвергаются. В 13 добавлено `maxModules` — число
  слотов апгрейда КОРПУСА: оно приходит из таблицы классов (1 у капсулы, 20 у
  крепости), а не выводится, и до этого round-trip возвращал всем бортам
  конструкторский дефолт 3. В 14 — `miningRig`, буровая установка.
- ⚠️ **Правило для любого нового поля корабля, которое пишет модуль.** Бонусы
  ЗАПЕКАЮТСЯ в поля (CHROMOCORE §2.4), а список `modules` при загрузке заново НЕ
  применяется. Поэтому такое поле обязано быть (а) записано в сейв — иначе тихо
  исчезнет при загрузке; (б) обнулено в `shipApplyClass` — иначе переживёт и
  пересадку на другой корпус, и ГИБЕЛЬ борта (`downgradeAgentToEscapePod` чистит
  список модулей, но поля перезаписывает именно `shipApplyClass`). Ни то, ни
  другое не выводится автоматически; на этом уже трижды ловились разные поля.
- **Энергия реактора не только на движение.** `shipReactorPower` — паспортная
  мощность установки: тяга корпуса × КПД поставленного движка. Ни одна из двух
  величин не зависит от содержимого баков, иначе потребитель на борту глох бы от
  пустого бака рабочего тела, которое реактору не нужно. `shipDrawReactorEnergy`
  жжёт под это топливо тем же законом, что и полёт (`specificEnergy` смеси), но
  БЕЗ КПД струи, и той же золой отвечает (общий `spillAsh`). Первый потребитель —
  буровая (`shipDrillPower` = мощность × `1 + miningRig`).
- **Буровая — модуль, а не свойство корпуса.** `Ship::miningRig` заполняется
  только модулями слота `Special` (Mining Laser I/II, Ore Refinery Rig). Даёт
  ТЕМП, но не выгоду: входит в мощность, а не в энергию на тонну, поэтому быстрее
  = дороже в час при неизменной цене тонны.
- **У спасательной капсулы трюм 12, а не 0.** С нулём она не могла ни возить, ни
  добывать (бур упирался в «CARGO FULL» на первом же кадре), то есть потеря
  корабля была тупиком, а не наказанием.
...
себя, и уравнение самоссылочно. Пусть `D` — МЁРТВАЯ МАССА манёвра (корпус, груз,
экзотика и топливо, УЖЕ залитое в бункер), `A` — это залитое топливо, `dF` —
сколько топлива ещё надо долить, `W` — рабочее тело,
`x = deltaV * DELTAV_SCALE / ve`:
...
⚠️ **Топливо в бункере — это груз.** До §48 здесь стояло `P` = корпус и груз, а
топливо решалось из уравнения — то есть считался борт, у которого в бункере ровно
столько, сколько нужно на это плечо. Настоящие корпуса летают с полным бункером, а
он у стартового Hauler весит 220 при сухой массе 60. Замер: расход занижался в
**2.44 раза**, и 9…18% прибытий приходились на борт, которому нечем тормозить.
Прежняя формула — частный случай новой при `A = 0`; `k` от запаса не зависит,
поэтому предел достижимости не сдвинулся.
...
## Emergency Stop And Drifting

`STOP` — это не телепорт в ноль скорости, а торможение той же моделью: корабль
гасит скорость собственной тягой, честно жжёт ОБА расходника и по инерции
пролетает заметное расстояние (замер: 14 лет, 0.85 ly, 34 топлива и 25 рабочего
тела). Флаг режима — `targetStar == -2`.

Встав между систем, корабль больше НИ К ОДНОЙ не пристыкован: `currentStar`
становится -1. Это принципиально, потому что перемещение привязано к
КООРДИНАТАМ, а не к системам:

- маршрут считается от фактического положения корабля (было 9.6 ly от порта
  вылета вместо реальных 3.6 — корабль платил не за ту дистанцию);
- торговать из пустоты нельзя: рынок за световые годы больше не доступен;
- вылететь можно к ЛЮБОЙ системе — из пустоты плечо всегда прямое, потому что
  звёздный граф нужен лишь для выбора промежуточных портов, а порта под ногами
  нет.

Время торможения зависит от МАССЫ, и разброс огромный: ускорение равно
`min(конструктивный предел, тяга/масса)`, поэтому полностью гружёный корабль
гасит 0.109c за 15.9 года, а почти пустой — за 1.15. На скорости симуляции x1
это 16 секунд против одной, на x10 — полторы секунды против одной десятой.
Отсюда и ощущение «STOP останавливает мгновенно»: на лёгком корабле так и есть,
и это правильное поведение, а не баг.

Интерфейс обязан показывать торможение явно (`BRAKING 0.121C -> 0` в панели
корабля, `X BRAKING` в нижнем ряду). Без этого кнопка выглядит несработавшей:
линия маршрута пропадает сразу, а тяжёлый корабль ещё полтора десятка лет
гасит ход.
