...
int main(int argc, char** argv) {
    ...
    //
    // ⚠️ (§52) КРУТИТЬ ПООЧЕРЁДНО, А НЕ ВПЕРЕМЕШКУ. `rng` — глобал ПРОЦЕССА
    // (`game.cpp`), один на все миры сразу, а не поле партии. Прежний цикл
    // `g.update(); loaded.update();` заставлял две копии тянуть числа из общего
    // потока по очереди — и они расходились ВСЕГДА, каким бы верным ни был
    // сейв. Ровно на этом артефакте и стояло «сейв не доносит будущее» (§51.5:
    // казна 3.90836e9 против 3.86135e9): сломана была предпосылка проверки, а
    // не механика. Правильный круг — прогнать одну копию, вернуть генератор в
    // то состояние, с которым она стартовала, и прогнать вторую.
    const std::mt19937 rngAfterLoad = rng;
    for (int s = 0; s < after * 100; ++s) g.update(0.01);
    rng = rngAfterLoad;
    for (int s = 0; s < after * 100; ++s) loaded.update(0.01);
